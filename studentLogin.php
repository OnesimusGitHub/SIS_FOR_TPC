...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($_SESSION['login_attempts'] >= 3 && isset($_SESSION['lockout_time']) && time() < $_SESSION['lockout_time']) {
        ...
        // Query to fetch the account with matching email and check student status
        $sql = "SELECT l.shsstud_ID, l.shslogin_password, s.studstat 
                FROM tblshslogin l 
                INNER JOIN tblshsstudent s ON l.shsstud_ID = s.shsstud_ID 
                WHERE l.shslogin_email = ?";
        ...
        if ($stmt->num_rows > 0) {
            $stmt->bind_result($studentId, $storedPassword, $studstat);
            $stmt->fetch();

            // Check if the student is archived
            if ($studstat === "ARCHIVED") {
                $error = "Your account has been archived. Please contact the administrator.";
            } 
            // Compare plain text passwords
            elseif ($password === $storedPassword) { // Plain text comparison
                $_SESSION['student_id'] = $studentId; // Set the session variable for the logged-in student
                $_SESSION['login_attempts'] = 0; // Reset login attempts on successful login
                header('Location: studentPROFILE.php'); // Redirect to the profile page
                exit();
            } else {
