...
// Load email credentials from a configuration file
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        // Check if the email exists in tblshslogin
        $stmt = $pdo->prepare("SELECT shslogin_password FROM tblshslogin WHERE shslogin_email = :email");
        $stmt->bindParam(':email', $email, PDO::PARAM_STR);
        $stmt->execute();
        $user = $stmt->fetch(PDO::FETCH_ASSOC);

        if ($user) {
            $password = $user["shslogin_password"];
            ...
            try {
                ...
                $mail->SMTPAuth = true;
                $mail->Username = $config['email']; // Load email from config
                $mail->Password = $config['password']; // Your email password
                $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;
                $mail->Port = 587;

                // Recipients
                $mail->setFrom($config['email'], 'Admin'); // Use email from config
...
<body>
    <div class="headers">
        ...
        <div class="header2">
            <a href="studentLogin.html" class="back-link">
