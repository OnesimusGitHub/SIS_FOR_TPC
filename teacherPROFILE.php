...
if (!isset($_SESSION['teacherid']) || empty($_SESSION['teacherid'])) { // Use teacherid instead of teacher_id
    // Debugging: Log the issue if session variable is not set
    error_log("Redirecting to teacherLogin.php: teacherid is not set or empty.");
    header('Location: teacherLogin.php'); // Redirect to teacher login page
    exit();
}

// Debugging: Log the session variable for verification
error_log("Session teacherid: " . $_SESSION['teacherid']);
...
$teacherId = $_SESSION['teacherid']; // Use teacherid instead of teacher_id

// Fetch teacher details from teachrinf table
$sql = "SELECT teacherid, teachername, teachermidd, teacherlastname, teacherfield, strand_ID, grade, teacher_pfp, teacher_ca, teacher_age,teacher_contactno,teacher_dob,teacher_email,teacher_extensions, teacher_padd, teacher_sex 
        FROM teachrinf 
        WHERE teacherid = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param('i', $teacherId); // Bind as an integer
$stmt->execute();
$result = $stmt->get_result();
$teacher = $result->fetch_assoc();

if (!$teacher) {
    // Debugging: Log the issue if teacher details are not found
    error_log("Teacher details not found for teacherid: $teacherId");
    echo "Error: Teacher details not found.";
    exit();
}

// Decode the profile picture (assumes it's stored as a blob in the database)
$profilePicture = !empty($teacher['teacher_pfp']) ? 'data:image/jpeg;base64,' . base64_encode($teacher['teacher_pfp']) : 'default-profile.jpg';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['current_password'], $_POST['new_password'], $_POST['confirm_password'])) {
    ...
    if (strlen($newPassword) < 8 || !preg_match('/\d/', $newPassword)) {
        ...
        $sql = "SELECT loginpass FROM login WHERE teacherid = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param('i', $teacherId); // Bind as an integer
        $stmt->execute();
        ...
        if ($currentPassword === $storedPassword) { // Direct comparison for plain text passwords
            if ($newPassword !== $confirmPassword) {
                ...
                $sql = "UPDATE login SET loginpass = ? WHERE teacherid = ?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param('si', $newPassword, $teacherId); // Bind as strings
                if ($stmt->execute()) {
                    // Redirect to avoid form resubmission
                    header('Location: teacherPROFILE.php?success=1');
                    exit();
                } else {
...
<body>
    ...
    <div class="container">
        <div class="profile-header">
            ...
            <div>
                <h1><?php echo htmlspecialchars($teacher['teachername'] . ' ' . $teacher['teacherlastname']); ?></h1>
                <p>ID: <?php echo htmlspecialchars($teacher['teacherid']); ?></p>
            </div>
        </div>
        <div class="section">
            <div>
                <label for="first-name">First Name</label>
                <input type="text" id="first-name" value="<?php echo htmlspecialchars($teacher['teachername']); ?>" disabled>
                
                <label for="middle-name">Middle Name</label>
                <input type="text" id="middle-name" value="<?php echo htmlspecialchars($teacher['teachermidd']); ?>" disabled>
                
                <label for="last-name">Last Name</label>
                <input type="text" id="last-name" value="<?php echo htmlspecialchars($teacher['teacherlastname']); ?>" disabled>
                
                <label for="extensions">Extensions</label>
                <input type="text" id="extensions" value="<?php echo htmlspecialchars($teacher['teacher_extensions'] ?? 'N/A'); ?>" disabled>
                
                <label for="dob">Date of Birth</label>
                <input type="text" id="dob" value="<?php echo htmlspecialchars($teacher['teacher_dob']); ?>" disabled>
                
                <label for="age">Age</label>
                <input type="text" id="age" value="<?php echo htmlspecialchars($teacher['teacher_age'] ?? 'N/A'); ?>" disabled>
            </div>
            <div>
                <label for="current-address">Current Address</label>
                <input type="text" id="current-address" value="<?php echo htmlspecialchars($teacher['teacher_ca']); ?>" disabled>
                
                <label for="permanent-address">Permanent Address</label>
                <input type="text" id="permanent-address" value="<?php echo htmlspecialchars($teacher['teacher_padd'] ?? 'N/A'); ?>" disabled>
                
                <label for="contact-number">Contact Number</label>
                <input type="text" id="contact-number" value="<?php echo htmlspecialchars($teacher['teacher_contactno']); ?>" disabled>
                
                <label for="email">Email</label>
                <input type="text" id="email" value="<?php echo htmlspecialchars($teacher['teacher_email']); ?>" disabled>
                
                <label for="sex">Sex</label>
                <input type="text" id="sex" value="<?php echo htmlspecialchars($teacher['teacher_sex'] ?? 'N/A'); ?>" disabled>
            </div>
        </div>
        <div class="section">
