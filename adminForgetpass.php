...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        // Check if the email exists in tbladminlogin
        $stmt = $pdo->prepare("SELECT adminlogin_password FROM tbladminlogin WHERE adminlogin_email = :email");
        $stmt->bindParam(':email', $email, PDO::PARAM_STR);
        $stmt->execute();
        $user = $stmt->fetch(PDO::FETCH_ASSOC);

        if ($user) {
            $password = $user["adminlogin_password"];
...
<body>
    <div class="headers">
        ...
        <div class="header2">
            <a href="adminLogin.html" class="back-link">
