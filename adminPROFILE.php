...
$adminId = $_SESSION['admin_id']; // Use session variable to fetch the correct admin data

// Fetch admin details from tbladmin
$sql = "SELECT admin_ID, admin_email, admin_contactno, admin_fnam, admin_mname, admin_lname, admin_exten, admin_sex, admin_dob, admin_age, admin_caddress, admin_paddress, admin_pfp 
        FROM tbladmin 
        WHERE admin_ID = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param('i', $adminId); // Bind as an integer
$stmt->execute();
...
$profilePicture = !empty($admin['admin_pfp']) ? 'data:image/jpeg;base64,' . base64_encode($admin['admin_pfp']) : 'default-profile.jpg';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['current_password'], $_POST['new_password'], $_POST['confirm_password'])) {
    ...
    if (strlen($newPassword) < 8 || !preg_match('/\d/', $newPassword)) {
        ...
        $sql = "SELECT adminlogin_password FROM tbladminlogin WHERE admin_ID = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param('i', $adminId); // Bind as an integer
        $stmt->execute();
        ...
        if ($currentPassword === $storedPassword) { // Direct comparison for plain text passwords
            if ($newPassword !== $confirmPassword) {
                ...
                $sql = "UPDATE tbladminlogin SET adminlogin_password = ? WHERE admin_ID = ?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param('si', $newPassword, $adminId); // Bind as strings
                if ($stmt->execute()) {
...
<body>
        ...
        <div class="sub-main">
            <div class="sub-header">
                ...
                <div class="home-section">
                    <img class="img-home" src="TPC-IMAGES/realadmin-removebg-preview.png" alt="Home Icon" class="home-icon">
    ...
    <div class="container">
        <div class="profile-header">
            ...
            <div>
                <h1><?php echo htmlspecialchars($admin['admin_fnam'] . ' ' . $admin['admin_lname']); ?></h1>
        ...
        <div class="section">
            <div>
                ...
                <input type="text" id="first-name" value="<?php echo htmlspecialchars($admin['admin_fnam']); ?>" disabled>
                
                <label for="middle-name">Middle Name</label>
                <input type="text" id="middle-name" value="<?php echo htmlspecialchars($admin['admin_mname']); ?>" disabled>
                
                <label for="last-name">Last Name</label>
                <input type="text" id="last-name" value="<?php echo htmlspecialchars($admin['admin_lname']); ?>" disabled>
                
                <label for="dob">Date of Birth</label>
                <input type="text" id="dob" value="<?php echo htmlspecialchars($admin['admin_dob']); ?>" disabled>
                
                <label for="age">Age</label>
                <input type="text" id="age" value="<?php echo htmlspecialchars($admin['admin_age']); ?>" disabled>
            </div>
            <div>
                <label for="address">Current Address</label>
                <input type="text" id="address" value="<?php echo htmlspecialchars($admin['admin_caddress']); ?>" disabled>
                
                <label for="contact">Contact Number</label>
                <input type="text" id="contact" value="<?php echo htmlspecialchars($admin['admin_contactno']); ?>" disabled>
                
                <label for="email">Email</label>
                <input type="text" id="email" value="<?php echo htmlspecialchars($admin['admin_email']); ?>" disabled>
                
                <label for="sex">Sex</label>
                <input type="text" id="sex" value="<?php echo htmlspecialchars($admin['admin_sex']); ?>" disabled>
            </div>
        </div>
        <div class="section">
    ...
    <div id="changePasswordModal" class="modal">
        <div class="modal-content">
            ...
            <p>Please enter your new password for <?php echo htmlspecialchars($admin['admin_email']); ?>.</p>
            <?php if (isset($error)): ?>
